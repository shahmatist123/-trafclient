...
var _cUsersKhggcDesktopProjectsReactTrafMixClientNodeModulesNextDistCompiledRegeneratorRuntimeRuntimeJs = _interopRequireDefault(__webpack_require__(4051));
function _arrayLikeToArray(arr, len) {
    if (len == null || len > arr.length) len = arr.length;
    for(var i = 0, arr2 = new Array(len); i < len; i++)arr2[i] = arr[i];
    return arr2;
...
function _loadGetInitialProps() {
    _loadGetInitialProps = _asyncToGenerator(_cUsersKhggcDesktopProjectsReactTrafMixClientNodeModulesNextDistCompiledRegeneratorRuntimeRuntimeJs.default.mark(function _callee(App, ctx) {
        var ref, message, res, props, message1;
        return _cUsersKhggcDesktopProjectsReactTrafMixClientNodeModulesNextDistCompiledRegeneratorRuntimeRuntimeJs.default.wrap(function _callee$(_ctx) {
...
var spec_extension_response = __webpack_require__(3013);
...
async function middleware(req, ev) {
    const response = spec_extension_response.NextResponse.next();
    const { cookies  } = req;
    response.headers.set('Authorization', cookies.accessToken);
    response.headers.set('mode:', 'no-cors');
    ...
    if (!cookies.accessToken) {
        ...
        if (!nowValidUrl) {
            return spec_extension_response.NextResponse.redirect('/signup');
        }
    } else {
        ...
        if (!nowValidUrl) {
            return spec_extension_response.NextResponse.redirect('/auto-tasks');
        }
    }
    return spec_extension_response.NextResponse.next();
}
